...
function displayStudents() {
    ...
    for (let i = 0; i < students.length; i++) {
        ...
        console.log(`${s.name} (ID: ${s.id}) - Scores: ${s.scores.join(', ')} - Average: ${avg.toFixed(2)}`);
    }
}
...
function findAverageById() {
    ...
    for (let i = 0; i < students.length; i++) {
        if (students[i].id === id) {
            const avg = calculateAverage(students[i]);
            console.log(`${students[i].name}'s average score: ${avg.toFixed(2)}`);
            return;
        }
    }
